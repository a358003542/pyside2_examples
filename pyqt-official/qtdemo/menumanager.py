...
from PySide2.QtCore import (QByteArray, QDir, QEasingCurve, QFile, QFileInfo,
                            QLibraryInfo, QObject, QPointF, QProcess,
                            QProcessEnvironment,
                            QStandardPaths, Qt, QTextStream, QUrl)
...
class MenuManager(QObject):
        ...
        if self.assistantProcess.state() != QProcess.Running:
            ...
            if not self.assistantProcess.waitForStarted():
                QMessageBox.critical(None, "PyQt Demo",
                                     f"Could not start {app}\n you may check the source code on  {executable}.")
                return
    ...
    def readInfoAboutExample(self, example):
        ...
        self.info.setdefault(name, {})['filename'] = example.getAttribute(
            'filename')
        self.info[name]['dirname'] = example.parentNode.getAttribute('dirname')
        self.info[name]['changedirectory'] = example.getAttribute(
            'changedirectory')
    ...
    def createLeafMenu(self, el):
        ...
        if el.getAttribute('executable') != 'false':
            ...
            self.createLowRightLeafButton("Display", 405,
                                          MenuManager.LAUNCH_QML, infoButtonsIn,
                                          infoButtonsOut,
                                          None)
    ...
    def createMenu(self, category, type):
        ...
        while currentNode is not None:
            ...
            while currentNode is not None and i < maxExamples:
                ...
                anim.setKeyValueAt(0.50, QPointF(xOffset, (
                        i * ihp) + yOffset + Colors.contentStartY + (
                                                         10 * float(
                                                     i / 4.0))))
                anim.setKeyValueAt(0.60, QPointF(xOffset, (
                        i * ihp) + yOffset + Colors.contentStartY))
                anim.setKeyValueAt(0.70, QPointF(xOffset, (
                        i * ihp) + yOffset + Colors.contentStartY + (
                                                         5 * float(
                                                     i / 4.0))))
                anim.setKeyValueAt(0.80, QPointF(xOffset, (
                        i * ihp) + yOffset + Colors.contentStartY))
                anim.setKeyValueAt(0.90, QPointF(xOffset, (
                        i * ihp) + yOffset + Colors.contentStartY + (
                                                         2 * float(
                                                     i / 4.0))))
                anim.setEndValue(QPointF(xOffset, (
                        i * ihp) + yOffset + Colors.contentStartY))
                movieIn.append(anim)
                ...
                anim.setStartValue(QPointF(xOffset, (
                        i * ihp) + yOffset + Colors.contentStartY))
                ...
                anim.setStartValue(QPointF(xOffset, (
                        i * ihp) + yOffset + Colors.contentStartY))
                anim.setKeyValueAt(0.55, QPointF(xOffset, (
                        i * ihp) + yOffset + Colors.contentStartY - i * 2.0))
                anim.setKeyValueAt(0.70, QPointF(xOffset - 10, (
                        i * ihp) + yOffset + Colors.contentStartY - i * 1.5))
                anim.setKeyValueAt(0.80, QPointF(xOffset, (
                        i * ihp) + yOffset + Colors.contentStartY - i * 1.0))
                anim.setKeyValueAt(0.90, QPointF(xOffset - 2, (
                        i * ihp) + yOffset + Colors.contentStartY - i * 0.5))
                anim.setEndValue(QPointF(xOffset, (
                        i * ihp) + yOffset + Colors.contentStartY))
                ...
                anim.setStartValue(QPointF(xOffset, (
                        i * ihp) + yOffset + Colors.contentStartY))
                anim.setKeyValueAt(0.70, QPointF(xOffset, (
                        maxExamples * ihp) + yOffset + Colors.contentStartY))
                anim.setEndValue(QPointF(-iw, (
                        maxExamples * ihp) + yOffset + Colors.contentStartY))
                ...
                anim.setKeyValueAt(0.30, QPointF(xOffset,
                                                 yOffset + Colors.contentStartY))
                anim.setEndValue(QPointF(xOffset, (
                        i * ihp) + yOffset + Colors.contentStartY))
            ...
            if currentNode is not None and i == maxExamples:
                ...
                # buttons.
                menuIndex += 1
                self.info.setdefault(currentMenu, {})['more'] = '%s -menu%d' % (
                    name, menuIndex)
                currentMenu = '%s -menu%d' % (name, menuIndex)
                self.info.setdefault(currentMenu, {})['back'] = '%s -menu%d' % (
                    name, menuIndex - 1)
    ...
    def createLowLeftButton(self, label, type, movieIn, movieOut, movieShake,
        ...
        buttonIn.setKeyValueAt(0.5, QPointF(-iw,
                                            Colors.contentStartY + Colors.contentHeight - 35))
        buttonIn.setKeyValueAt(0.7, QPointF(xOffset,
                                            Colors.contentStartY + Colors.contentHeight - 35))
    ...
    def createLowRightButton(self, label, type, movieIn, movieOut, movieShake):
        ...
        anim.setKeyValueAt(0.5, QPointF(sw,
                                        Colors.contentStartY + Colors.contentHeight - 35))
        anim.setKeyValueAt(0.7, QPointF(xOffset + 535,
                                        Colors.contentStartY + Colors.contentHeight - 35))
        anim.setEndValue(QPointF(xOffset + 535,
        ...
        anim.setStartValue(QPointF(xOffset + 535,
                                   Colors.contentStartY + Colors.contentHeight - 26))
        anim.setEndValue(
    ...
    def createLowRightLeafButton(self, label, xOffset, type, movieIn, movieOut,
        ...
        anim.setStartValue(
            QPointF(sw, Colors.contentStartY + Colors.contentHeight - 35))
        anim.setKeyValueAt(0.10, QPointF(sw,
                                         Colors.contentStartY + Colors.contentHeight - 35))
        anim.setKeyValueAt(0.30, QPointF(xOffset,
                                         Colors.contentStartY + Colors.contentHeight - 35))
        anim.setKeyValueAt(0.35, QPointF(xOffset + 30,
                                         Colors.contentStartY + Colors.contentHeight - 35))
        anim.setKeyValueAt(0.40, QPointF(xOffset,
                                         Colors.contentStartY + Colors.contentHeight - 35))
        anim.setKeyValueAt(0.45, QPointF(xOffset + 5,
                                         Colors.contentStartY + Colors.contentHeight - 35))
        anim.setKeyValueAt(0.50, QPointF(xOffset,
                                         Colors.contentStartY + Colors.contentHeight - 35))
        anim.setEndValue(
    ...
    def createInfo(self, item, name):
        ...
        infoIn.setStartValue(QPointF(self.window.scene.sceneRect().width(),
    ...
    def createTicker(self):
        ...
        self.tickerInAnim.setKeyValueAt(0.60, QPointF(qtendpos,
                                                      Colors.contentStartY + qtPosY))
        self.tickerInAnim.setKeyValueAt(0.70, QPointF(qtendpos + 30,
                                                      Colors.contentStartY + qtPosY))
        self.tickerInAnim.setKeyValueAt(0.80, QPointF(qtendpos,
                                                      Colors.contentStartY + qtPosY))
        self.tickerInAnim.setKeyValueAt(0.90, QPointF(qtendpos + 5,
                                                      Colors.contentStartY + qtPosY))
        ...
        qtOut.setEndValue(QPointF(self.window.scene.sceneRect().width() + 700,
                                  Colors.contentStartY + qtPosY))
        ...
        qtActivate.setStartValue(QPointF(self.window.scene.sceneRect().width(),
                                         Colors.contentStartY + qtPosY))
        qtActivate.setKeyValueAt(0.60, QPointF(qtendpos,
                                               Colors.contentStartY + qtPosY))
        qtActivate.setKeyValueAt(0.70, QPointF(qtendpos + 30,
                                               Colors.contentStartY + qtPosY))
        qtActivate.setKeyValueAt(0.80, QPointF(qtendpos,
                                               Colors.contentStartY + qtPosY))
        qtActivate.setKeyValueAt(0.90, QPointF(qtendpos + 5,
                                               Colors.contentStartY + qtPosY))
    ...
    def createUpnDownButtons(self):
        ...
        self.downButton.setPos(
            xOffset + 10 + self.downButton.sceneBoundingRect().width(), yOffset)
