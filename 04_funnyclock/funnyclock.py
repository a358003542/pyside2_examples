...
from PySide2.QtGui import QColor, QPalette, QBrush, QPixmap, QPolygon, QPainter
from PySide2.QtCore import QPoint, QTimer, Qt
from PySide2.QtWidgets import QWidget, QApplication, QAction
...
class Clock(QWidget):
    ...
    def handChange(self):
        ...
        self.hand = (max(self.side / 200, 4), max(self.side / 100, 8),
                     max(self.side / 40, 30))
        self.hourHand = QPolygon([QPoint(self.hand[0], self.hand[1]),
                                  QPoint(-self.hand[0], self.hand[1]),
                                  QPoint(0, -self.hand[2])])
        self.minuteHand = QPolygon([QPoint(self.hand[0], self.hand[1]),
                                    QPoint(-self.hand[0], self.hand[1]),
                                    QPoint(0, -self.hand[2] * 1.6)])
        self.secondHand = QPolygon([QPoint(self.hand[0], self.hand[1]),
                                    QPoint(-self.hand[0], self.hand[1]),
